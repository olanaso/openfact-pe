...
public class SunatSignerUtils {
    ...
    public static Element getSignToElement(OpenfactSession session, OrganizationModel organization) {
        Document signedDocument = sign(session, organization);
    ...
    public static Document sign(OpenfactSession session, OrganizationModel organization) {
        ...
        try {
            ...
            DOMSignContext domSignCtx = new DOMSignContext(keystore.getActiveRsaKey(organization).getPrivateKey(),
                    document);
