...
public class SunatUBLRetentionProvider implements UBLRetentionProvider {
	...
	public UBLSender<RetentionModel> sender() {
		return new UBLSender<RetentionModel>() {
			...
			@Override
			public SendEventModel sendToCustomer(OrganizationModel organization, RetentionModel retention) throws SendException {
				/*
			...
			@Override
			public SendEventModel sendToThridParty(OrganizationModel organization, RetentionModel retention) throws SendException {
				...
				try {
					...
					model = session.getProvider(SunatSendEventProvider.class).addSendEvent(organization, SendResultType.SUCCESS, retention);
					...
					model = session.getProvider(SunatSendEventProvider.class).addSendEvent(organization, SendResultType.ERROR, retention);
					model.addFileAttatchments(SunatTemplateUtils.toFileModel(InternetMediaType.ZIP, fileName, zip));
					model.setDestiny(SunatSenderUtils.getDestiny(EmissionType.OCPE));
					model.setType("SUNAT");
			...
			@Override
			public SendEventModel sendToThridParty(OrganizationModel organization, RetentionModel retentionModel, SendEventModel sendEvent) throws SendException {
				return null;
			}
		};
	}
