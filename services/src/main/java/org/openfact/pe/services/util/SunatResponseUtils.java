...
public class SunatResponseUtils {
    ...
    private static void writeByteToFile(String fileName, byte[] document, boolean response) throws IOException {
        if (!response) {
            FileUtils.writeByteArrayToFile(new File(System.getProperty("user.home") + "/ubl/" + fileName + ".zip"), document);
        } else {
            FileUtils.writeByteArrayToFile(new File(System.getProperty("user.home") + "/ubl/R" + fileName + ".zip"), document);
        }
    }
