...
public class SunatUBLPerceptionProvider implements UBLPerceptionProvider {
    ...
    public UBLWriter<PerceptionType> writer() {
        return new UBLWriter<PerceptionType>() {
            ...
            @Override
            public Document write(OrganizationModel organization, PerceptionType perceptionType, Map<String, List<String>> attributes) {
                try {
    ...
    public UBLSender<PerceptionModel> sender() {
        return new UBLSender<PerceptionModel>() {
            ...
            @Override
            public SendEventModel sendToCustomer(OrganizationModel organization, PerceptionModel perception) throws SendException {
                SendEventModel sendEvent = perception.addSendEvent(DestinyType.CUSTOMER);
                return sendToCustomer(organization, perception, sendEvent);
            }

            @Override
            public SendEventModel sendToCustomer(OrganizationModel organization, PerceptionModel perception, SendEventModel sendEvent) throws SendException {
                /*
                ...
                try {
                    ...
                    sendEvent = session.getProvider(SunatSendEventProvider.class).addSendEvent(organization, SendResultType.SUCCESS, perception);
                    ...
                    sendEvent = session.getProvider(SunatSendEventProvider.class).addSendEvent(organization, SendResultType.ERROR, perception);
            ...
            @Override
            public SendEventModel sendToThridParty(OrganizationModel organization, PerceptionModel perception) throws SendException {
                ...
                try {
                    ...
                    model = session.getProvider(SunatSendEventProvider.class).addSendEvent(organization, SendResultType.SUCCESS, perception);
                    ...
                    model = session.getProvider(SunatSendEventProvider.class).addSendEvent(organization, SendResultType.ERROR, perception);
                    model.addFileAttatchments(SunatTemplateUtils.toFileModel(InternetMediaType.ZIP, fileName, zip));
                    model.setDestiny(SunatSenderUtils.getDestiny(EmissionType.OCPE));
                    model.setType("SUNAT");
            ...
            @Override
            public SendEventModel sendToThridParty(OrganizationModel organization, PerceptionModel perceptionModel, SendEventModel sendEvent) throws SendException {
                return null;
            }
        };
    }
