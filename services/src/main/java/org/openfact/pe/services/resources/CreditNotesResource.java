...
public class CreditNotesResource {
    ...
    public CreditNotesResource(OpenfactSession session, OrganizationModel organization, AdminEventBuilder adminEvent) {
    ...
    public Response createCreditNote(DocumentRepresentation rep) {
        ...
        if (rep.getSerie() != null || rep.getNumero() != null) {
            if (rep.getSerie() != null && rep.getSerie() != null) {
                if (creditNoteManager.getCreditNoteByDocumentId(organization, rep.getSerie() + "-" + rep.getNumero()) != null) {
        ...
        try {
            ...
            CreditNoteModel creditNoteModel = creditNoteManager.addCreditNote(organization, creditNoteType, generateAttributes(rep));

            if (rep.isEnviarAutomaticamenteASunat()) {
                SendEventModel thirdPartySendEvent = creditNoteModel.addSendEvent(DestinyType.THIRD_PARTY);
                try {
                    creditNoteManager.sendToTrirdParty(organization, creditNoteModel, thirdPartySendEvent);
            ...
            if (rep.isEnviarAutomaticamenteAlCliente()) {
                SendEventModel customerSendEvent = creditNoteModel.addSendEvent(DestinyType.CUSTOMER);
                try {
                    creditNoteManager.sendToCustomerParty(organization, creditNoteModel, customerSendEvent);
                } catch (ModelInsuficientData e) {
            ...
            URI location = session.getContext().getUri().getAbsolutePathBuilder().path(creditNoteModel.getId()).build();
            adminEvent.operation(OperationType.CREATE).resourcePath(location.toString()).representation(rep).success();
            return Response.created(location).entity(ModelToRepresentation.toRepresentation(creditNoteModel)).build();
    ...
    public Response getCdr(@PathParam("creditNoteId") final String creditNoteId) {
        ...
        if (creditNote == null) {
            return ErrorResponse.error("Credit Note not found", Response.Status.NOT_FOUND);
        }

        Map<String, String> params = new HashMap<>();
        params.put(CreditNoteModel.SEND_EVENT_DESTINY_TYPE, DestinyType.THIRD_PARTY.toString());
        params.put(CreditNoteModel.SEND_EVENT_RESULT, SendResultType.SUCCESS.toString());

        List<SendEventModel> sendEvents = creditNote.searchForSendEvent(params, 0, 2);
