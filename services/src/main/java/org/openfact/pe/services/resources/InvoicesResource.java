...
public class InvoicesResource {
    ...
    public InvoicesResource(OpenfactSession session, OrganizationModel organization, AdminEventBuilder adminEvent) {
    ...
    public Response createInvoice(DocumentRepresentation rep) {
        ...
        try {
            ...
            InvoiceModel invoiceModel = invoiceManager.addInvoice(organization, invoiceType, generateAttributes(rep));

            if (rep.isEnviarAutomaticamenteASunat()) {
                SendEventModel thirdPartySendEvent = invoiceModel.addSendEvent(DestinyType.THIRD_PARTY);
                try {
                    invoiceManager.sendToTrirdParty(organization, invoiceModel, thirdPartySendEvent);
            ...
            if (rep.isEnviarAutomaticamenteAlCliente()) {
                SendEventModel customerSendEvent = invoiceModel.addSendEvent(DestinyType.CUSTOMER);
                try {
                    invoiceManager.sendToCustomerParty(organization, invoiceModel, customerSendEvent);
                } catch (ModelInsuficientData e) {
            ...
            URI location = session.getContext().getUri().getAbsolutePathBuilder().path(invoiceModel.getId()).build();
            adminEvent.operation(OperationType.CREATE).resourcePath(location.toString()).representation(rep).success();
            return Response.created(location).entity(ModelToRepresentation.toRepresentation(invoiceModel)).build();
    ...
    public Response getCdr(@PathParam("invoiceId") final String invoiceId) {
        ...
        params.put(InvoiceModel.SEND_EVENT_DESTINY_TYPE, DestinyType.THIRD_PARTY.toString());
        params.put(InvoiceModel.SEND_EVENT_RESULT, SendResultType.SUCCESS.toString());

        List<SendEventModel> sendEvents = invoice.searchForSendEvent(params, 0, 2);
